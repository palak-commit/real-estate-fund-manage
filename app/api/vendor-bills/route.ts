...
const SELECT = `SELECT b.id, DATE_FORMAT(b.txn_date, '%Y-%m-%d') AS txn_date,
    ...
    b.paid_to, b.amount, b.gst, b.total_bill, b.note, b.status,
...
export async function POST(req: NextRequest) {
  ...
  const [res]: any = await pool.query(
    `INSERT INTO vendor_bills (txn_date, project_id, category_id, paid_to, amount, gst, total_bill, note, status)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [d.txn_date || null, d.project_id, d.category_id || null, d.paid_to || null, d.amount, d.gst, total, d.note || null, d.status]
  );
  ...
  await logActivity({
    ...
    title: `Vendor bill added${d.paid_to ? ` · ${d.paid_to}` : ""}`,
    amount: total,
    meta: { total_bill: total, status: d.status },
  });
